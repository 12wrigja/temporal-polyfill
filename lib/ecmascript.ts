...
export function DifferenceTemporalPlainYearMonth(
  ...
  const otherFields = ToTemporalYearMonthFields(other, fieldNames) as Required<Temporal.PlainYearMonthLike> & {
    day: number;
  };
  otherFields.day = 1;
  const thisFields = ToTemporalYearMonthFields(yearMonth, fieldNames) as Required<Temporal.PlainYearMonthLike> & {
    day: number;
  };
  thisFields.day = 1;
...
export function AddDurationToOrSubtractDurationFromPlainYearMonth(
  ...
  const calendar = GetSlot(yearMonth, CALENDAR);
  const fieldNames = CalendarFields(calendar, ['monthCode', 'year'] as const);
  const fields = ToTemporalYearMonthFields(yearMonth, fieldNames) as Required<Temporal.PlainYearMonthLike> & {
    day: number;
  };
  const sign = DurationSign(years, months, weeks, days, 0, 0, 0, 0, 0, 0);
