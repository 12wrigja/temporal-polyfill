...
export function ToSecondsStringPrecision(options: Temporal.ToStringPrecisionOptions): {
  ...
  // This condition is impossible according to type-checking, so we have to
  // widen the return from GetTemporalUnit to trick tsc.
  // This could potentially be gated behind DEBUG to remove unnecessary code iff
  // users are comfortable relying on the typechecker instead.
  if ((smallestUnit as Temporal.TimeUnit) === 'hour') {
...
function GetOption<P extends keyof O, O extends Partial<Record<P, unknown>>>(
  options: O,
  property: P,
...
>(
  ...
  if (value !== undefined) {
    ...
    if (!allowedValues.includes(value)) {
      throw new RangeError(`${String(property)} must be one of ${allowedValues.join(', ')}, not ${value}`);
    }
    return value;
  }
  return fallback;
...
function GetNumberOption<P extends keyof T, T extends Partial<Record<P, unknown>>>(
  options: T,
  property: P,
  minimum: number,
  maximum: number,
